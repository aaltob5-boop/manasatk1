...
function route() {
  ...
  // Guard: admin/student areas require a session tied to a code that is
  // still active. The session itself lives in localStorage so it survives
  // closing the browser/app — the user stays logged in across visits and
  // is only signed out automatically once their code actually expires
  // or is stopped by the admin.
  if (parts[0] === "admin" || parts[0] === "student") {
    if (!session || session.role !== parts[0]) {
      location.hash = "#/login";
      return;
    }
    if (!sessionCodeValid(session)) {
      Session.clear();
      toast("انتهت صلاحية اشتراكك أو تم إيقاف الكود. سجّل الدخول مرة أخرى أو جدّد الاشتراك", "red");
      location.hash = "#/login";
      return;
    }
  }
...
// Re-check the session every couple of minutes and whenever the tab
// becomes visible again, so a subscription that expires while the app
// is left open still logs the user out promptly instead of only on
// the next manual navigation.
setInterval(route, 120000);
...
function openCodeModal() {
  openModal(`
    <h3>إنشاء كود جديد لطالب</h3>
    <p class="sub">اكتب بيانات الطالب هنا مباشرة — الطالب هيدخل بالكود على طول من غير ما يعمل تسجيل بنفسه.</p>
    <div class="form-grid">
      ...
      <div class="field"><label>اسم الطالب *</label><input id="newStudentName" /></div>
      <div class="field"><label>رقم هاتف الطالب *</label><input id="newStudentPhone" type="tel" /></div>
      <div class="field"><label>رقم ولي الأمر *</label><input id="newStudentParentPhone" type="tel" /></div>
      <div class="field"><label>المدرسة *</label><input id="newStudentSchool" /></div>
      <div class="field"><label>المحافظة *</label><input id="newStudentGov" /></div>
      <div class="field">
        <label>الشعبة *</label>
        <select id="newStudentTrack">
          <option value="علمي علوم">علمي علوم</option>
          <option value="علمي رياضة">علمي رياضة</option>
          <option value="أدبي">أدبي</option>
        </select>
      </div>
      <div class="field"><label>البريد الإلكتروني (اختياري)</label><input id="newStudentEmail" type="email" /></div>
    </div>
    <div class="modal-close-row">
...
function createCode() {
  ...
  const name = val("newStudentName"), phone = val("newStudentPhone"), parentPhone = val("newStudentParentPhone");
  const school = val("newStudentSchool"), gov = val("newStudentGov"), track = val("newStudentTrack"), email = val("newStudentEmail");

  if (!code) return toast("أدخل نص الكود", "red");
  if (!name || !phone || !parentPhone || !school || !gov) return toast("من فضلك أكمل بيانات الطالب المطلوبة", "red");
  if (DB.col("codes").some((c) => c.code.toUpperCase() === code)) return toast("هذا الكود مستخدم بالفعل", "red");

  const newCodeObj = DB.add("codes", {
    code, type: "student", studentName: name, startDate: todayISO(),
  ...
  const student = DB.add("students", {
    name, phone, parentPhone, school, governorate: gov, track, email,
    codeId: newCodeObj.id, deviceId: null, avatar: null, createdAt: todayISO(),
    videosWatched: [], examsCompleted: []
  });

  DB.update("codes", newCodeObj.id, { studentId: student.id });

  toast("تم إنشاء الكود وربط بيانات الطالب بنجاح");
  closeModal();
  route();
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Easy manual reset: visiting the site once with ?reset=1 in the URL
  // wipes any old cached data (old codes/admin password from a previous
  // version) and reseeds fresh data. Useful after updating the code/app
  // so visitors' browsers don't keep stale localStorage forever.
  if (new URLSearchParams(location.search).get("reset") === "1") {
    DB.reset();
    toast("تم تحديث بيانات المنصة بنجاح", "blue");
