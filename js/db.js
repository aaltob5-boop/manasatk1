...
// ---- session validity vs. subscription code ----
// A session should stay logged in (persisted in localStorage) for as long
// as its underlying code is still active and, for student codes, not past
// its endDate. This lets the browser tab/app be closed and reopened days
// later without needing to log in again, while still enforcing that access
// stops the moment the subscription actually expires.
function sessionCodeValid(session) {
  ...
  if (code.endDate && daysBetween(todayISO(), code.endDate) < 0) {
    // subscription date has passed — flip the code to expired so the
    // admin panel reflects it too, then reject the session
